...
export function transportOptions(from,to,dna,adjustment={}){
  ...
  const options=[];
  if(distance<=3&&!adjustment.tired)options.push(option('步行','walking',walk,walk,5));
  options.push(option('大眾運輸（待查）','transit',transit,8,35),option('車輛（待查）','driving',car,3,20));
...
export function directionsUrl(a,b,travelMode){
  const url=new URL('https://www.google.com/maps/dir/');url.searchParams.set('api','1');
  url.searchParams.set('origin',`${a.lat},${a.lng}`);url.searchParams.set('destination',`${b.lat},${b.lng}`);
  if(['walking','transit','driving'].includes(travelMode))url.searchParams.set('travelmode',travelMode);
  return url.toString();
}
